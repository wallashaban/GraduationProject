...
class ReportRemoteDataSource implements BaseReportRemoteDataSource{
  Dio? dio;
  String token='eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL2JhYnktaGVhbHRoLWNhcmUuc29uaWNhci50ZWNoL2FwaS9hdXRoL2xvZ2luIiwiaWF0IjoxNjgxNTkzMzU4LCJuYmYiOjE2ODE1OTMzNTgsImp0aSI6InBHWGRRa2g2UmtHbTE2SnYiLCJzdWIiOiIxMSIsInBydiI6IjIzYmQ1Yzg5NDlmNjAwYWRiMzllNzAxYzQwMDg3MmRiN2E1OTc2ZjcifQ.nhTGq_UBowmJ40Sg1JEVOERfS2cluwA3xyK_x6qt2VI';
  ...
  Future<List<VaccinationReportModel>> vaccinationsReport() async{
    final Response response =
        await dio!.get(AppConstants.vaccinationReport,);
  ...
  Future<LatestDevelopmentModel> latestDevelopments() async{
    final Response response =
        await dio!.get(AppConstants.latestDevelopment,);
    if (response.data['status'] == true) {
      debugPrint('LatestDevelopment   remote data ${response.data}');//['data']['questions']why not??
      return LatestDevelopmentModel.fromJson(response.data['data']);
    } else {
      throw ServerException(
        errorMessageModel: ErrorMessageModel.fromJson(response.data),
      );
  ...
  Future<List<DiseaseReportModel>> diseasesReport() async{
    final Response response =
        await dio!.get(AppConstants.diseaseReport,);
    if (response.data['status'] == true) {
  ...
  Future<LatestTeethModel> latestTeeeth() async{
    final Response response =
        await dio!.get(AppConstants.latestTeeth,);
    if (response.data['status'] == true) {
  ...
  Future<MedicalInfoModel> medicalsInfo() async{
    final Response response =
        await dio!.get(AppConstants.medicalInfo,);
    if (response.data['status'] == true) {
